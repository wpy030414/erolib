...
export function AppToast() {
  ...
  // 视觉顺序表（活动 + 离场项的 id）：离场项保留原槽位，兄弟不跳位——
  // Vue TransitionGroup「离场保持在流内」的语义。
  ...
  if (prevRef.current !== toasts) {
    ...
    const activeIds = toasts.map((t) => t.id);
    const goneIds = new Set(gone.map((t) => t.id));
    orderRef.current = [
      ...orderRef.current.filter((id) => activeIds.includes(id) || goneIds.has(id)),
      ...activeIds.filter((id) => !orderRef.current.includes(id)),
    ];
    if (gone.length) {
      const ids = goneIds;
  ...
  const byId = new Map<number, ToastMessage>();
  for (const m of toasts) byId.set(m.id, m);
  for (const m of leavingOnly) byId.set(m.id, m);
  const leavingIds = new Set(leavingOnly.map((m) => m.id));
  // 渲染顺序：按视觉顺序表；尚未入表的新项（首帧）按 store 顺序补在后面。
  const ordered = [
    ...orderRef.current.map((id) => byId.get(id)).filter((m): m is ToastMessage => !!m),
    ...[...byId.values()].filter((m) => !orderRef.current.includes(m.id)),
  ];

  return (
    <div className="toast-container">
      {ordered.map((msg) => {
        const isLeaving = leavingIds.has(msg.id);
        return (
          <div
            key={msg.id}
